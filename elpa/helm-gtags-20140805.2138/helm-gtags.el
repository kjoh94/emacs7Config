...
(defvar helm-gtags-max-stack-size  10)
...
;; kjoh's note [ 2016-04-24 Sun 09:59 ]
(defvar helm-gtags-action-at-one-if-one nil)
...
(defun helm-gtags--get-context-info ()
  (let* ((tag-location (helm-gtags-find-tag-directory))
    ...
    (if (null context-stack)
        (progn
          (message "Context stack is empty(TAG at %s)" tag-location)
...
(defun helm-gtags--move-to-context (context)
  (let ((file (plist-get context :file))
        ...
        (prev (buffer-file-name (current-buffer))))
    (push-mark)                         ; kjoh's note
...
(defun helm-gtags--push-context (context)
   (let* ((context-info (helm-gtags--get-or-create-context-info))
     ...
     ;; kjoh's note
     ;; check 'context' belong to helm-gtags-tag-location
     (if (not (string-match (concat "^" (file-truename helm-gtags-tag-location)) (file-truename file)))
         nil                            ; do nothing
       (unless (= current-index -1)
         (setq context-stack (nthcdr (1+ current-index) context-stack)))
       (setq helm-gtags--current-position nil)
       (let* ((top (pop context-stack))
              (file1 (plist-get top :file))
              (line1 (plist-get top :line)))
         (if (and (string= file file1) (string= line line1))
             (push top context-stack)
           (if top
               (push top context-stack))
           (push context context-stack)
           (if (> (length context-stack) helm-gtags-max-stack-size)
               (nbutlast context-stack))))
       (helm-gtags--put-context-stack helm-gtags-tag-location -1 context-stack))))
...
;;;;;;;;;;;;;;;;;;;;;;
;; kjoh's note
;; added comparing prev-buffer-name with curr-buffer-name
;; if different, jump with highlight
;; else jump with no decoration
;; and, added push-mark on previous place
...
(defun helm-gtags-show-stack-init ()
  (cl-loop with context-stack = (plist-get (helm-gtags--get-context-info) :stack)
           ...
           ;;for context in (reverse context-stack)  ;; kjoh's note: delete reverse option
...
(defvar helm-source-gtags-tags
  `((name . "GNU GLOBAL")
    ...
    ;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
    ;; [2016-04-24 Sun 09:17] kjoh's note
    ;; open file in other window
    ;;(action . helm-gtags-action-openfile)
    (action . (("Goto the location" . helm-gtags-action-openfile)
               ("Goto the location(other buffer)" .
                (lambda (elm)
                  (setq helm-gtags-use-otherwin t)
                  (helm-gtags-action-openfile elm)))))))
...
(defun helm-gtags--show-stack-action (index)
  (let* ((context-info (helm-gtags--get-context-info))
    ...
    ;; (helm-gtags--put-context-stack helm-gtags-tag-location
    ;;                                index context-stack)
    (helm-gtags--move-to-context (nth index ;; context-stack       ;; kjoh's note
...
(defun helm-gtags-common (srcs)
  (let ((helm-quit-if-no-candidate t)
        (helm-execute-action-at-once-if-one helm-gtags-action-at-one-if-one)
...
;; kjoh
(defun kjoh/helm-gtags-save-current-context ()
  (interactive)
  (push-mark)
  (helm-gtags-find-tag-directory)
  (helm-gtags-save-current-context)
